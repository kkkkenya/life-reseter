...
- **Today** — opens on one decision: the Most Important Task as a big hero card
  (tap to close it out), the day's first quest as a quiet line under it, the streak
  strip, and everything else — task list, schedule, full quest board — one pull below.
  This is the only screen you have to open daily.
- **Calendar** — the whole picture on one month grid: blocks, classes, deadlines,
  and your Google Calendar when connected.
...
The app is mobile-first, but above the `lg` breakpoint it uses the full window: a
persistent left navigation rail replaces the bottom tab bar and page content widens.
Nothing changes below `lg` — the phone layout is untouched.
