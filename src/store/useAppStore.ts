      ...
      merge: (persisted, current) => {
        ...
        return {
          ...
          profile: {
            ...current.profile,
            ...p,
            tools: { ...current.profile.tools, ...(p.tools ?? {}) },
            goals: (() => {
              ...
              (Object.keys(EMPTY_GOALS) as LifeAreaKey[]).forEach((key) => {
                ...
                merged[key] = { ...EMPTY_GOALS[key], ...current.profile.goals[key], ...(saved ?? {}) };
              });
              return merged;
            })(),
            pinnedFocusArea: p.pinnedFocusArea ?? null,
            lastGoalsReviewAt: p.lastGoalsReviewAt ?? null,
            streaks: Array.isArray(p.streaks)
              ? p.streaks.map((h) => ({ ...h, mode: h.mode ?? "avoidance" }))
              : [],
            ...
            budgetCategories: Array.isArray(p.budgetCategories) && p.budgetCategories.length > 0
              ? p.budgetCategories
