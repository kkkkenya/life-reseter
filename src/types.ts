...
export interface JournalEntry {
  ...
  sleepHours?: number; // last night, logged with the morning check-in
  workoutMinutes?: number; // 0 = rest day
  examen?: ExamenEntry;
...
export interface UserProfile {
  ...
  xpTotal: number;
  pomodoroSessions: number; // focus sessions logged from the (re-addable) Pomodoro
  streaks: StreakHabit[];
